...
export function initDiscordBot({ q, uid, onLog = console, sseSendAll } = {}) {
  ...
  async function ensureGorjetaPublicMessage(keyId, payload) {
  const channelId = String(process.env.DISCORD_GORJETA_PUBLIC_CHANNEL_ID || '').trim();
  if (!channelId) return;
  ...
  await q(`
  CREATE TABLE IF NOT EXISTS gorjeta_discord_public (
    id TEXT PRIMARY KEY,
    channel_id TEXT,
    message_id TEXT,
    updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
...
try {
  await q(`ALTER TABLE gorjeta_discord_public RENAME COLUMN round_id TO id`);
} catch {}

  const id = String(keyId || "gorjeta-main");
  const prev = await q(`SELECT message_id FROM gorjeta_discord_public WHERE id=$1`, [id]);
  const mid = prev?.rows?.[0]?.message_id ? String(prev.rows[0].message_id) : null;

  const sendPayload = payload && typeof payload === "object" ? payload : { content: String(payload || "") };
  ...
  if (!mid) {
    ...
    await q(
      `INSERT INTO gorjeta_discord_public (id, channel_id, message_id, updated_at)
       VALUES ($1,$2,$3,now())
       ON CONFLICT (id) DO UPDATE SET channel_id=$2, message_id=$3, updated_at=now()`,
      [id, String(channelId), String(sent.id)]
    );
    return;
  }
  ...
  if (!msg) {
    ...
    await q(
      `UPDATE gorjeta_discord_public SET channel_id=$2, message_id=$3, updated_at=now() WHERE id=$1`,
      [id, String(channelId), String(sent.id)]
    );
    return;
  }

  await msg.edit(sendPayload).catch(() => null);
  await q(`UPDATE gorjeta_discord_public SET updated_at=now() WHERE id=$1`, [id]);
}

async function sendGorjetaBatchSummary({
  roundId,
  batchId,
  perWinnerCents,
  winnersCountReq,
  ...
  await q(`
    CREATE TABLE IF NOT EXISTS gorjeta_discord_batch_messages (
  ...
  const fmt = (c) => (Number(c || 0) / 100).toLocaleString("pt-BR", { style: "currency", currency: "BRL" });

  const aprovados = [];
  const reprovados = [];

  for (const row of results || []) {
    const nick = String(row.twitch_name || "").trim();
    ...
    if (status === "CONFIRMADO" || status === "APROVADO") {
      aprovados.push(`✅ ${nick} — Aprovado`);
    } else {
      reprovados.push(`❌ ${nick} — Não aprovado${reason ? ` (${reason})` : ""}`);
    }
  }

  function fitLines(lines, emptyText = "—") {
  ...
  const sent = await ch.send({
    embeds: [{
      title: "🎁 Gorjeta — Sorteio",
      description: `Rodada **${short(roundId)}** • Lote **${short(batchId)}**`,
      fields: [
        { name: "Por ganhador", value: fmt(perWinnerCents), inline: true },
        { name: "Pedidos", value: String(winnersCountReq), inline: true },
        { name: "Confirmados", value: String(confirmedCount), inline: true },
        { name: "Desclassificados", value: String(disqCount), inline: true },
        { name: "Gasto", value: fmt(spentCents), inline: true },
        { name: "Saldo", value: fmt(remainingCents), inline: true },
        { name: "Aprovados", value: fitLines(aprovados), inline: false },
        { name: "Não aprovados", value: fitLines(reprovados), inline: false }
      ]
    }]
  }).catch(() => null);
  ...
  await q(
    `INSERT INTO gorjeta_discord_batch_messages (batch_id, round_id, channel_id, message_id, created_at)
     VALUES ($1,$2,$3,$4,now())
     ON CONFLICT (batch_id)
     DO UPDATE SET round_id=$2, channel_id=$3, message_id=$4`,
    [String(batchId), String(roundId), String(channelId), String(sent.id)]
  ).catch(() => {});
}

async function clearGorjetaBatchSummaries() {
  const channelId = String(process.env.DISCORD_GORJETA_PUBLIC_CHANNEL_ID || '').trim();
  if (!channelId) return;
  ...
  const mainRef = await q(
    `SELECT message_id
       FROM gorjeta_discord_public
      WHERE id=$1
      LIMIT 1`,
    ['gorjeta-main']
  ).catch(() => null);

  if (mainRef?.rows?.[0]?.message_id) {
    mainMessageId = String(mainRef.rows[0].message_id);
  }

  const tracked = await q(
    `SELECT channel_id, message_id
       FROM gorjeta_discord_batch_messages
  ...
  for (const row of tracked.rows || []) {
    const cid = String(row.channel_id || channelId);
    const mid = String(row.message_id || '');
    if (!mid || seen.has(mid) || mid === mainMessageId) continue;
  ...
  if (ch && ch.isTextBased()) {
    ...
    for (let page = 0; page < 10; page++) {
      const pack = await ch.messages.fetch({ limit: 100, ...(before ? { before } : {}) }).catch(() => null);
      if (!pack || !pack.size) break;

      const list = [...pack.values()];

      for (const msg of list) {
        if (!msg?.id || seen.has(msg.id) || msg.id === mainMessageId) continue;
        if (!msg.author || msg.author.id !== client.user.id) continue;

        const title = String(msg.embeds?.[0]?.title || '').toLowerCase();
        const isBatch = title.includes('gorjeta') && title.includes('sorteio');
  ...
  await q(`DELETE FROM gorjeta_discord_batch_messages`).catch(() => {});
}



async function sendGorjetaPaymentsQR(confirmados) {
  const channelId = String(process.env.DISCORD_GORJETA_PAGAMENTOS_CHANNEL_ID || '').trim();
  if (!channelId) return;
  ...
  await q(`
    CREATE TABLE IF NOT EXISTS gorjeta_discord_payment_messages (
  ...
  for (const w of confirmados || []) {
    const pagamentoId = String(w.pagamento_id || '').trim();
    const chave = String(w.pix_key || '').trim();
    const payload = String(w.pix_payload || '').trim();

    if (!pagamentoId || !chave || !payload) continue;
    ...
    const msg = await ch.send({
      content:
        `💸 **GORJETA (PAGAR)**\n` +
        `Nick: **${w.twitch_name}**\n` +
        `Valor: **${fmt(w.valor_cents)}**\n` +
        `Pagamento ID: \`${pagamentoId}\``,
      files: [{ attachment: png, name: `gorjeta-${w.twitch_name}.png` }]
    }).catch(() => null);

    if (!msg?.id) continue;

    await q(
      `INSERT INTO gorjeta_discord_payment_messages (pagamento_id, channel_id, message_id, created_at)
       VALUES ($1,$2,$3,now())
       ON CONFLICT (pagamento_id)
       DO UPDATE SET channel_id=$2, message_id=$3`,
      [pagamentoId, String(channelId), String(msg.id)]
    ).catch(() => {});
  }
}


async function deleteGorjetaPaymentMessage(pagamentoId) {
  ...
  const ref = await q(
    `SELECT channel_id, message_id
       FROM gorjeta_discord_payment_messages
      WHERE pagamento_id = $1
      LIMIT 1`,
    [id]
  ).catch(() => null);

  const row = ref?.rows?.[0] || null;

  if (row?.channel_id && row?.message_id) {
    const ch = await client.channels.fetch(String(row.channel_id)).catch(() => null);
    if (ch && ch.isTextBased()) {
      const msg = await ch.messages.fetch(String(row.message_id)).catch(() => null);
      if (msg) {
        await msg.delete().catch(() => {});
      }
    }
  ...
  await q(
    `DELETE FROM gorjeta_discord_payment_messages
      WHERE pagamento_id = $1`,
    [id]
  ).catch(() => {});
}
...
try {
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    try {
      if (interaction.isButton()) {
        ...
        if (interaction.customId.startsWith('gorjeta_ver_mais:')) {
  const canView =
  ...
  if (!canView) {
    await interaction.reply({
      flags: 64,
      content: '❌ Só moderadores podem usar esse botão.'
    }).catch(() => {});
    return;
  }
  ...
  if (!rows.length) {
    const m = await interaction.channel.send({
      content: `👥 ${interaction.user} ainda não tem participantes na gorjeta.`
    }).catch(() => null);

    if (m) setTimeout(() => m.delete().catch(() => {}), 3 * 60 * 1000);
    return;
  }

  const lines = rows.map(row => {
    const nick = String(row.twitch_name || '').trim();
    const ok = String(row.approval_status || '').toUpperCase() === 'APROVADO';
    return `${ok ? '🟢' : '🔴'} ${nick} — ${ok ? 'Aprovado' : 'Não aprovado'}`;
  });

  const chunks = [];
  let current = `👥 **Participantes (${lines.length})** — pedido por ${interaction.user}\n\n`;
  ...
  for (const part of chunks) {
    const m = await interaction.channel.send({ content: part }).catch(() => null);
    if (m) setTimeout(() => m.delete().catch(() => {}), 3 * 60 * 1000);
  }
...
client.on('shardReady', (id) => onLog.log('✅ [DISCORD] shardReady:', id));
  ...
  updateEntryMessage,
  ensureGorjetaPublicMessage,
  sendGorjetaBatchSummary,
  sendGorjetaPaymentsQR,
  deleteGorjetaPaymentMessage,
  clearGorjetaBatchSummaries
};
}
